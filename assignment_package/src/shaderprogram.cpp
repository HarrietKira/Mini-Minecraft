...
ShaderProgram::ShaderProgram(OpenGLContext *context)
    : vertShader(), fragShader(), prog(),
      ...
      unif_sampler2D(-1), unif_time(-1), unif_post_type(-1),
...
void ShaderProgram::create(const char *vertfile, const char *fragfile)
{
    ...
    unif_post_type = context->glGetUniformLocation(prog, "u_post_type");

    // Find corresponding variables in shader
...
void ShaderProgram::draw_interleaved(Drawable &d, int texture_slot = 0)
{
    useMe();
    ...
    if(unif_sampler2D != -1)
    {
        context->glUniform1i(unif_sampler2D, /*GL_TEXTURE*/texture_slot);
    }
...
void ShaderProgram::draw_interleaved_transparent(Drawable &d, int texture_slot = 0)
{
    useMe();

    if(d.elem_count_transparent() < 0) {
        throw std::out_of_range("Attempting to draw a drawable with m_count_transparent of " + std::to_string(d.elem_count_transparent()) + "!");
    }
    ...
    // Bind the index buffer and then draw shapes from it.
    // This invokes the shader program, which accesses the vertex buffers.
    d.bind_idx_transparent();
    context->glDrawElements(d.drawMode(), d.elem_count_transparent(), GL_UNSIGNED_INT, 0);
...
//This function, as its name implies, uses the passed in GL widget
void ShaderProgram::draw_quad(Drawable &d)
{
    useMe();
...
void ShaderProgram::set_time(int t)
{
    useMe();
...
void ShaderProgram::set_post_type(int type)
{
    useMe();

    if(unif_post_type != -1)
    {
        context->glUniform1i(unif_post_type, type);
    }
}
