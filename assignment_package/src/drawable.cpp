...
Drawable::Drawable(OpenGLContext* context)
    : m_count(-1), m_bufIdx(), m_bufPos(), m_bufNor(), m_bufCol(), m_bufUV(),
      m_idxGenerated(false), m_posGenerated(false), m_norGenerated(false), m_colGenerated(false), m_UVGenerated(),
      mp_context(context)
{}
...
void Drawable::destroyVBOdata()
{
    ...
    mp_context->glDeleteBuffers(1, &m_buf_idx_transparent);
    mp_context->glDeleteBuffers(1, &m_buf_data_transparent);

    mp_context->glDeleteBuffers(1, &m_bufUV);

    m_idxGenerated = m_posGenerated = m_norGenerated = m_colGenerated = m_idx_generated_transparent = m_data_generated_transparent = m_UVGenerated = false;
    m_count = -1;
    m_count_transparent = -1;
}
...
int Drawable::elem_count_transparent()
{
...
void Drawable::generate_idx_transparent()
{
    m_idx_generated_transparent = true;
    // Create a VBO on our GPU and store its handle in bufIdx
    mp_context->glGenBuffers(1, &m_buf_idx_transparent);
}

void Drawable::generate_data_transparent()
{
    m_data_generated_transparent = true;
    // Create a VBO on our GPU and store its handle in bufPos
    mp_context->glGenBuffers(1, &m_buf_data_transparent);
}
...
bool Drawable::bind_idx_transparent()
{
    if(m_idx_generated_transparent) {
        mp_context->glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, m_buf_idx_transparent);
    }
    return m_idx_generated_transparent;
}

bool Drawable::bind_data_transparent()
{
    if(m_data_generated_transparent){
        mp_context->glBindBuffer(GL_ARRAY_BUFFER, m_buf_data_transparent);
    }
    return m_data_generated_transparent;
}
